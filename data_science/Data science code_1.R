...
ggplot(crop_recommendation, aes(x=rainfall, y=yield_kg_ha)) + 
  geom_point() + 
  labs(title= "Scatter plot", x= "rainfall", y= "yield_kg_ha")
...
#add color
ggplot(crop_recommendation, aes(x=rainfall, y=Nitrogen, color = Crop)) + 
  geom_point() + 
  labs(title= "Scatter plot", x= "rainfall", y= "Nitrogen")
...
ggplot(crop_recommendation, aes(x=yield_kg_ha))+ 
  ...
  labs(title= "Histogram of crop yield")  
# draw a histogram for rainfall
ggplot(crop_recommendation, aes(x=rainfall))+ 
  geom_histogram() +  
  labs(title= "Histogram of rainfall") 
#rainfall result is skewed to the left
...
ggplot(crop_recommendation, aes(x=yield_category, fill =Crop)) + 
  ...
  labs(title ="Yield category", x="Yield category")+
   coord_flip() 
#cross tabulation 
table(crop_recommendation$Crop,crop_recommendation$yield_category)
...
#test for PH DISTRIBUTION 
shapiro.test(crop_recommendation$Ph)
...
##ANOVA 
#ANOVA tests can be performed using the "aov" function and the "lm" function.
...
ANOVA1 <- lm(Pesticide_kg~SoilType, data=Kenya_agri_disease_spatial)
summary(ANOVA1)

#Exercise : Test whether mean pesticide is equal among LandCover categories


#wilcoxson's test for equality of medians
#check for normality of rainfall
shapiro.test(crop_recommendation$rainfall)
#perform wilcoxon test
wilcox.test(rainfall ~ yield_category, data = crop_recommendation)

#transform Crop variable into a factor
crop_recommendation$Crop <- as.factor(crop_recommendation$Crop)
...
library(FSA)
#
dunnTest(rainfall ~ Crop, data = crop_recommendation, method = "holm", altp=TRUE)
